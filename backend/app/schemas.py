...
class JobRequestSchema(Schema):
    ...
    job_type = fields.Str(
        required=True,
        validate=validate.OneOf(["mechanic", "spareshop", "rescue"])
    )
...
class MessageSchema(Schema):
    ...
    message_type = fields.Str(
        load_default="text",
        validate=validate.OneOf(["text", "image", "voice", "file"])
    )


class AdminNotifySchema(Schema):
    message = fields.Str(required=True, validate=validate.Length(min=1))
    target = fields.Str(load_default=None, allow_none=True)   # specific user email/id
    role = fields.Str(load_default=None, allow_none=True)     # driver | mechanic | spareshop
