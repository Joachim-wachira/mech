...
def register_driver():
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"[register_driver] DB error: {exc}")
        return error_response("Could not create account — please try again later.", 500)
...
def register_mechanic():
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"[register_mechanic] DB error: {exc}")
        return error_response("Could not create account — please try again later.", 500)
    return _auth_response(user)
...
def register_spareshop():
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"[register_spareshop] DB error: {exc}")
        return error_response("Could not create account — please try again later.", 500)
    return _auth_response(user)
...
def change_password():
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"[change-password] {exc}")
        return error_response("Could not update password", 500)
