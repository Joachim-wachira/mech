...
# ── Subscription ──────────────────────────────────────────────────
# Tracks platform subscription payments for mechanics and spare shops.
# Drivers use the platform free. Only mechanics and spareshops must subscribe.
#
# Plans:
#   monthly  — 1 month from payment date
#   annual   — 12 months from payment date
#   biennial — 5 years (60 months) from payment date
#
# Flow:
#   1. User registers (is_subscribed=False, subscription_status='unpaid')
#   2. Admin sees them in subscription queue — unverified until subscribed
#   3. User uploads proof of payment or admin marks as paid
#   4. Admin can approve, setting is_subscribed=True + expiry date
#   5. Cron / before_request check flags expiring subscriptions
#   6. Admin can grant a grace period (1 week or 1 month) on expiry
#   7. Admin can suspend account on expiry with no grace

PLAN_DURATIONS = {
    "monthly":  30,
    "annual":   365,
    "biennial": 1825,   # 5 years
}
...
class Subscription(db.Model):
    """Platform subscription record for a mechanic or spare shop."""
    ...
    # Plan chosen
    plan            = db.Column(db.String(20), nullable=False, default="monthly")  # monthly|annual|biennial

    # Status lifecycle
    # unpaid → pending_verification → active → grace | expired | suspended
    status          = db.Column(db.String(30), nullable=False, default="unpaid")

    # Dates
    paid_at         = db.Column(db.DateTime, nullable=True)          # when payment was confirmed
    starts_at       = db.Column(db.DateTime, nullable=True)          # subscription start
    expires_at      = db.Column(db.DateTime, nullable=True)          # subscription end
    grace_until     = db.Column(db.DateTime, nullable=True)          # if admin granted grace period

    # Payment tracking (integration-ready — connect to M-Pesa / manual proof)
    amount_kes      = db.Column(db.Float, nullable=True)
    payment_method  = db.Column(db.String(40), default="manual")     # manual|mpesa
    payment_ref     = db.Column(db.String(120), nullable=True)       # M-Pesa transaction code
    proof_url       = db.Column(db.String(400), nullable=True)       # screenshot of payment
    ...
    # Notification tracking (so we don't spam)
    notified_7d     = db.Column(db.Boolean, default=False)   # 7-day expiry warning sent
    notified_1d     = db.Column(db.Boolean, default=False)   # 1-day expiry warning sent

    created_at      = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at      = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)

    user      = db.relationship("User", foreign_keys=[user_id], backref=db.backref("subscription", uselist=False))
