...
# ── Call log (for calls tab) ─────────────────────────────────────
@routes_bp.route("/calls/log", methods=["GET"])
@jwt_required()
def calls_log():
    from app.models import CallLog
    user = current_user()
    calls = CallLog.query.filter(
        db.or_(CallLog.caller_id == user.id, CallLog.callee_id == user.id)
    ).order_by(CallLog.created_at.desc()).limit(50).all()
    result = []
    for c in calls:
        caller = User.query.get(c.caller_id)
        callee = User.query.get(c.callee_id) if c.callee_id else None
        result.append({
            "id":               c.id,
            "caller_id":        c.caller_id,
            "callee_id":        c.callee_id,
            "caller_name":      caller.full_name if caller else "Unknown",
            "callee_name":      callee.full_name if callee else "Unknown",
...
# ── Send message via REST (fallback when WebSocket is unavailable) ──
@routes_bp.route("/conversations/<int:conv_id>/messages", methods=["POST"])
@jwt_required()
def send_message_rest(conv_id):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"[send_message_rest] {exc}")
    ...
    try:
        ...
        current_app.logger.warning(f"[send_message_rest] WS emit failed: {e}")

    return success_response({"message": msg.to_dict() if hasattr(msg, 'to_dict') else {"id": msg.id}}, status=201)
